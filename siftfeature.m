...
for aa = 1:size(dog_space,2)
    ...
    for bb = k + 1: 1: row(aa) - k -1
        for cc = k + 1: 1: col(aa) - k - 1
            ...
            if (main2(bb, cc) == max(main2_window(:)))...
                    && (main2(bb, cc) >= max(neighof2_1window(:)))...
                    && (main2(bb, cc) >= max(neighof2_2window(:)))
                
                coord = [coord; 2 aa bb cc sigma_arr(2,aa)];
                
            elseif (main2(bb, cc) == min(main2_window(:)))...
                    && (main2(bb, cc) <= min(neighof2_1window(:)))...
            ...
            if (main3(bb, cc) == max(main3_window(:)))...
                    && (main3(bb, cc) >= max(neighof3_1window(:)))...
                    && (main3(bb, cc) >= max(neighof3_2window(:)))
                
                coord = [coord; 3 aa bb cc sigma_arr(3,aa)];
                
            elseif (main3(bb, cc) == min(main3_window(:)))...
                    && (main3(bb, cc) <= min(neighof3_1window(:)))...
...
for ll=1:size(coord,1)
    ...
    Dx = (D(coord(ll,3), coord(ll,4) + 1)...
        - D(coord(ll,3), coord(ll,4) - 1))/2;
    Dy = (D(coord(ll,3) + 1, coord(ll,4))...
        - D(coord(ll,3) - 1, coord(ll,4)))/2;
    Ds = (dog_space{coord(ll,1) + 1, coord(ll,2)}...
        (coord(ll,3), coord(ll,4))...
        - dog_space{coord(ll,1) - 1, coord(ll,2)}...
        (coord(ll,3), coord(ll,4)))/2;
    Dxx = D(coord(ll, 3), coord(ll, 4) + 1)...
        + D(coord(ll, 3), coord(ll, 4) - 1)...
        - (2*D(coord(ll, 3), coord(ll, 4)));
    Dyy = D(coord(ll, 3) + 1, coord(ll, 4))...
        + D(coord(ll, 3) - 1, coord(ll, 4))...
        - (2*D(coord(ll, 3), coord(ll, 4)));
    Dxy = (D(coord(ll, 3) + 1, coord(ll, 4) + 1)...
        - D(coord(ll, 3) - 1, coord(ll, 4) + 1)...
        -D(coord(ll, 3) + 1, coord(ll, 4) - 1)...
        + D(coord(ll, 3) - 1, coord(ll, 4) - 1))/4;
    Dxs = (dog_space{coord(ll,1) + 1, coord(ll,2)}...
        (coord(ll, 3), coord(ll, 4) + 1)...
        - dog_space{coord(ll,1) + 1, coord(ll,2)}...
        (coord(ll, 3), coord(ll, 4) - 1)...
        - dog_space{coord(ll,1) - 1, coord(ll,2)}...
        (coord(ll, 3), coord(ll, 4) + 1)...
        + dog_space{coord(ll,1) - 1, coord(ll,2)}...
        (coord(ll, 3), coord(ll, 4) - 1))/4;
    Dys = (dog_space{coord(ll,1) + 1, coord(ll,2)}...
        (coord(ll, 3) + 1, coord(ll, 4))...
        - dog_space{coord(ll,1) + 1, coord(ll,2)}...
        (coord(ll, 3) - 1, coord(ll, 4))...
        - dog_space{coord(ll,1) - 1, coord(ll,2)}...
        (coord(ll, 3) + 1, coord(ll, 4))...
        + dog_space{coord(ll,1) - 1, coord(ll,2)}...
        (coord(ll, 3) - 1, coord(ll, 4)))/4;
    Dss = (dog_space{coord(ll,1) + 1, coord(ll,2)}...
        (coord(ll, 3), coord(ll, 4))...
        + dog_space{coord(ll,1) - 1, coord(ll,2)}...
        (coord(ll, 3), coord(ll, 4)))...
        - (2*D(coord(ll, 3), coord(ll, 4)));
    ...
    if (abs(Taylor_dog) > threshold_1) && (max(abs(loc_ext)) < 0.5)
        coord2 = [coord2; coord(ll,:) (coord(ll,3)+loc_ext(1))...
            (coord(ll,4)+loc_ext(2)) (coord(ll,5)+loc_ext(3)) Taylor_dog];
    end
...
for ll = 1:size(coord2,1)
    ...
    C_dog = 0.015;
    if (abs(coord2(ll, 9)) >= C_dog)...
            && (abs(D(coord2(ll, 3), coord2(ll, 4))) >=  0.8*C_dog)
...
for ll=1:size(coord3,1)
    ...
    Dxx_new = D(coord3(ll, 3), coord3(ll, 4) + 1)...
        + D(coord3(ll, 3), coord3(ll, 4) - 1)...
        - (2*D(coord3(ll, 3), coord2(ll, 4)));
    Dxy_new = (D(coord3(ll, 3) + 1, coord3(ll, 4) + 1)...
        - D(coord3(ll, 3) - 1, coord3(ll, 4) + 1)...
        -D(coord3(ll, 3) + 1, coord3(ll, 4) - 1)...
        + D(coord3(ll, 3) - 1, coord3(ll, 4) - 1))/4;
    Dyy_new = D(coord3(ll, 3) + 1, coord3(ll, 4))...
        + D(coord3(ll, 3) - 1, coord3(ll, 4))...
...
for oo=1:size(coord_final,1)
    ...
    gauss_filter = fspecial('gaussian',...
        [winsize winsize], 1.5*coord_final(oo,8));
    currx = coord_final(oo,3) ;%* (2^(coord_final(oo, 2) -idy)); 
    curry = coord_final(oo,4) ;%* (2^(coord_final(oo, 2) -idy));
    
    theta_win = theta_I(currx-(winsize-1)/2+offset:...
        currx+(winsize-1)/2+offset,...
        curry-(winsize-1)/2+offset:...
        curry+(winsize-1)/2+offset);
    mag_win = mag_I(currx-(winsize-1)/2+offset:...
        currx+(winsize-1)/2+offset,...
        curry-(winsize-1)/2+offset:...
        curry+(winsize-1)/2+offset);
    ...
    for ii = 1:size(theta_win,1)
        for jj = 1:size(theta_win,2)
            ...
            m = mag_win(ii,jj);
            hist_bin(1, floor(theta/10)+1) = ...
                hist_bin(1, floor(theta/10)+1) + m;
        end
    ...
    for aa = 1:length(hist_bin)
        if hist_bin(aa) >= 0.8*maxEl 
            ...
            coord_final2 = ...
                [coord_final2; coord_final(oo,:) idx idy hist_bin(aa) dir];
        end
    end
end
        ...
        for j = 1:w:16
            ...
            for k = 1:4
                for l = 1:4                                    
                    ...
                    feature_vec(oo, count+floor(theta_bin/45)) =...
                        feature_vec(oo, count+floor(theta_bin/45))...
                        + mag_f(k,l);
